<script lang="ts">
  ...
  import { BITMASK_KEYS, KEY_NOTES, decodeBits, describeMessageId } from "../../shared/knowledge.ts";
  import { menuTrigger, copyText, scan, type MenuItem } from "./state.svelte.ts";
  ...
  function buildMenu(): { title: string; items: MenuItem[] } {
    ...
    items.push({
      label: "Compare across country bundles",
...
{#if matches}
  <div>
    ...
    {#if showHex && isBlob(value)}
      <pre class="code hex">{(value.__data.match(/.{1,32}/g) ?? []).map((g) => (g.match(/.{1,2}/g) ?? []).join(" ")).join("\n")}</pre>
    {/if}
