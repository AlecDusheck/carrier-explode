...
The tool also decodes the Intel-heritage `.der.pri` / `.der.gri` carrier files that the C1 consumes.
...
python3 ftab.py pri      overrides_V59.der.pri # carrier PRI / global GRI -> key = value rows
python3 ftab.py fetch    <ipsw-url> ftab.bin   # copy the modem ftab out of a remote IPSW (HTTP Range)
```
...
the rcpi/HWID and board tables, digest verification, fwsg/CAR section tables, PRI/GRI decoding,
and a Python-only path the site's build scripts can import.
...
| `.der.pri` / `.der.gri` (C1/Intel dialect) | DER `SET { [0] SEQ { [6000] name, [6001] value } … }` header rows (Carrier ID, PRI Revision, PRI Name / GRI Revision). Then `[0] SEQ { [6002] key, [6003] value }` items. Keys are `"%u:<nvm.path>"`, `"%s[N]:…"` (string) or `"%qu[N]:…"`. Values are raw little-endian or NUL-terminated text. Tags `9fae70..73`. | high |
| `.der.pri` (Qualcomm dialect) | tags `9fa711…` (5009+) with a compressed EFS payload. Only the header is decoded here. | n/a |
